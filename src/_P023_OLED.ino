...
enum { OLED_64x48 = 0x01, OLED_rotated = 0x02 };
...
boolean Plugin_023(byte function, struct EventStruct *event, String &string) {
  ...
  case PLUGIN_WEBFORM_LOAD: {
    ...
    LoadCustomTaskSettings(event->TaskIndex, (byte *)&deviceTemplate,
                           sizeof(deviceTemplate));
    for (byte varNr = 0; varNr < 8; varNr++) {
      string += F("<TR><TD>Line ");
      string += varNr + 1;
      string += F(":<TD><input type='text' size='64' maxlength='64' "
                  "name='Plugin_023_template");
      string += varNr + 1;
      string += F("' value='");
    ...
    addPinSelect(false, string, "taskdevicepin3",
                 Settings.TaskDevicePin3[event->TaskIndex]);

    char tmpString[128];

    sprintf_P(tmpString, PSTR("<TR><TD>Display Timeout:<TD><input type='text' "
                              "name='plugin_23_timer' value='%u'>"),
  ...
  case PLUGIN_WEBFORM_SAVE: {
    ...
    for (byte varNr = 0; varNr < 8; varNr++) {
      ...
      strncpy(deviceTemplate[varNr], tmpString.c_str(),
              sizeof(deviceTemplate[varNr]));
    }

    Settings.TaskDeviceID[event->TaskIndex] =
        1; // temp fix, needs a dummy value

    SaveCustomTaskSettings(event->TaskIndex, (byte *)&deviceTemplate,
                           sizeof(deviceTemplate));
    success = true;
    break;
  }
...
static void Plugin_023_sendCharXY(unsigned char data, int X, int Y) {
  // if (interrupt && !doing_menu) return; // Stop printing only if interrupt is
  // call but not in button functions
...
static void Plugin_023_setXY(unsigned char row, unsigned char col) {
  ...
  Plugin_023_sendcommand(0x10 + ((8 * col >> 4) & 0x0f)); // set high col
                                                          // address
}
